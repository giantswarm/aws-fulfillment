...
import (
	...
	"github.com/giantswarm/aws-fulfillment/customer"
)
...
func (m *Mock) FetchCustomerData(token string) (customer.Data, error) {
	log.Printf("Fetching mock customer data from AWS")
...
func (c *AWS) FetchCustomerData(token string) (customer.Data, error) {
	log.Printf("Fetching customer data from AWS")

	customerData := customer.Data{}

	log.Printf("Resolving customer")
	...
	log.Printf("Getting entitlements")
	...
	for _, entitlement := range getEntitlementsResults.Entitlements {
		...
		customerData.Entitlements = append(customerData.Entitlements, customer.Entitlements{
